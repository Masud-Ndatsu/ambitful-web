...
export default function OpportunityDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FB] max-h-screen overflow-y-scroll">
      {/* Header */}
      <div className="bg-white border-b border-[#E3E3E3] sticky top-0 z-10">
        <div className="max-w-[1400px] mx-auto px-8 py-6">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-[1.4rem] text-[#505662] hover:text-[#1A1D23] transition-colors"
          >
      ...
      <div className="max-w-[1400px] mx-auto px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-8 border border-[#E3E3E3]">
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                  <h1 className="text-[2.8rem] font-bold text-[#1A1D23] mb-2">
                    {opportunity.title}
                  </h1>
                  <div className="flex items-center gap-4 text-[1.4rem] text-[#505662] mb-4">
                  ...
                  <div className="flex items-center gap-3">
                    {opportunity.opportunityCategories.map((cat) => (
                      <Badge
                        ...
                        className="bg-[#F6F8FB] text-[#1A1D23] border-0 text-[1.2rem] px-3 py-1"
                      >
                        {cat.opportunityType.name}
                      </Badge>
                    ))}
                    {opportunity.compensation && (
                      <Badge className="bg-[#E7F5E1] text-[#007947] border-0 text-[1.2rem] px-3 py-1 font-semibold">
                        {opportunity.compensation}
                      </Badge>
                    )}
                  </div>
                ...
                {/* Apply Button */}
                <Button
                  className="bg-[#007947] hover:bg-[#00df82] text-white text-[1.4rem] font-semibold px-8 py-6 rounded-xl transition-colors"
                  onClick={() => {
            ...
            {/* About the Role */}
            <div className="bg-white rounded-2xl p-8 border border-[#E3E3E3]">
              <h2 className="text-[2rem] font-bold text-[#1A1D23] mb-4">
                About the Role
              </h2>
              <p className="text-[1.6rem] text-[#505662] leading-relaxed whitespace-pre-wrap">
                ...
                <div className="bg-white rounded-2xl p-8 border border-[#E3E3E3]">
                  ...
                  <ul className="space-y-3">
                    {opportunity.requirements.map((req, index) => (
                      <li
                        key={index}
                        className="flex items-start gap-3 text-[1.6rem] text-[#505662]"
                      >
            ...
            {opportunity.benefits && opportunity.benefits.length > 0 && (
              <div className="bg-white rounded-2xl p-8 border border-[#E3E3E3]">
                <h2 className="text-[2rem] font-bold text-[#1A1D23] mb-6">
                  Benefits & Perks
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {opportunity.benefits.map((benefit, index) => (
                    ...
                    >
                      ...
                      <span className="text-[1.4rem] text-[#1A1D23] font-medium">
                        {benefit}
                      </span>
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 border border-[#E3E3E3]">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-[1.2rem] text-[#505662] uppercase font-semibold">
                  Application Deadline
                </span>
              </div>
              <p className="text-[1.6rem] font-bold text-[#1A1D23]">
            ...
            <Button
              variant="outline"
              className="w-full border-[#E3E3E3] text-[#1A1D23] text-[1.4rem] font-semibold py-6 rounded-xl flex items-center justify-center gap-2 hover:bg-[#F6F8FB]"
              onClick={() => {
        ...
        <div className="mt-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[2.4rem] font-bold text-[#1A1D23]">
              Similar Opportunities
            </h2>
            <Button
              variant="link"
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[1.6rem] font-semibold text-[#1A1D23] mb-1 truncate">
                        {opp.title}
                      </h3>
                  ...
                  <div className="flex items-center gap-2 text-[1.2rem] text-[#505662] mb-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {opp.opportunityCategories.slice(0, 2).map((cat) => (
                      <Badge
                        key={cat.opportunityType.id}
                        className="bg-[#F6F8FB] text-[#1A1D23] border-0 text-[1rem] px-2 py-1"
                      >
                        {cat.opportunityType.name}
                      </Badge>
                    ))}
